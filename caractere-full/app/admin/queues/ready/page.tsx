...
export default function ReadyQueuePage() {
  ...
  const handleSendToYalidine = async (orderId: string) => {
    ...
    if (!error) {
      setOrders(orders.map(o => o.id === orderId ? { ...o, status: 'livree' } : o));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-8">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Commandes Prêtes</h1>
          ...
          {orders.length === 0 ? (
            ...
            orders.map(order => {
              ...
              return (
                <div
                  key={order.id}
                  className="bg-white dark:bg-slate-900 rounded-lg border border-green-200 dark:border-green-900 p-6 hover:shadow-md transition-shadow"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex gap-3 mt-3">
                        <span className="inline-block px-2 py-1 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 text-xs rounded">
                          {statusDef?.label}
                        </span>
                        {order.yalidine_tracking && (
                          <span className="inline-block px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs rounded">
                            Suivi: {order.yalidine_tracking}
                          </span>
                        )}
                      </div>
                    </div>
