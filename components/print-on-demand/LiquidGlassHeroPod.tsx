...
// Hero 3D liquid glass pour /print-on-demand — palette lime/black
  ...
  return (
    <>
      ...
      <Float speed={reduced ? 0 : 0.8} rotationIntensity={reduced ? 0 : 0.25} floatIntensity={reduced ? 0 : 0.5}>
        <GlassBlob position={[-2.5, 0.5, -1]} scale={1.7} speed={0.8} distortion={0.3} color="#A3E635" reduced={reduced} />
      </Float>
      <Float speed={reduced ? 0 : 1.05} rotationIntensity={reduced ? 0 : 0.25} floatIntensity={reduced ? 0 : 0.5}>
        <GlassBlob position={[2.6, -0.7, -1.6]} scale={1.3} speed={1.05} distortion={0.45} color="#5a5a5a" reduced={reduced} />
      </Float>
    </>
  )
}

export default function LiquidGlassHeroPod() {
