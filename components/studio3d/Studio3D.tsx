...
class ErrorBoundary extends Component<{ children: ReactNode }, { error: string | null }> {
  ...
  render() {
    if (this.state.error) return (
      <div className="flex min-h-screen items-center justify-center bg-[#071828] p-6 text-white">
...
function Loader() {
  ...
  return <mesh ref={m}><torusGeometry args={[0.12,0.03,8,24]}/><meshStandardMaterial color="#c9a84c"/></mesh>;
}
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-[#071828] text-white">
        <header className="flex items-center justify-between border-b border-white/10 bg-[#071828] px-5 py-3">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white">
              <span className="text-base font-black text-[#1a5f8a]">C</span>
            </div>
            <div>
              <p className="text-sm font-bold leading-tight">Caractère</p>
              <p className="text-[10px] leading-tight text-white/40">Studio 3D</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <a href="/" className="text-xs text-white/30 hover:text-white">← Retour</a>
          </div>
        </header>

        <div className="mx-auto flex max-w-6xl flex-col lg:flex-row">
          <div className="relative h-[56vh] min-h-[340px] flex-1 lg:h-[calc(100vh-57px)]">
            <div className="pointer-events-none absolute inset-0 z-10" style={{backgroundImage:"linear-gradient(rgba(255,255,255,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.03) 1px,transparent 1px)",backgroundSize:"40px 40px"}}/>
            <Canvas shadows camera={{position:[0,0,2.2],fov:25}} gl={{preserveDrawingBuffer:true,antialias:true}}
            ...
            {rec&&<div className="absolute left-4 top-4 z-20 flex items-center gap-2 rounded-full bg-black/60 px-3 py-1.5 backdrop-blur">
              <span className="h-2 w-2 animate-pulse rounded-full bg-red-500"/><span className="text-xs font-semibold tabular-nums">{recSec}s/{MAX_REC}s</span>
            </div>}
            {!logoTex&&<button onClick={()=>logoRef.current?.click()}
              className="absolute bottom-5 left-1/2 z-20 -translate-x-1/2 rounded-full border border-white/20 bg-black/40 px-5 py-2 text-xs font-medium backdrop-blur hover:bg-white/10">
              + Ajouter votre logo
            </button>}
          </div>

          <aside className="w-full border-t border-white/10 bg-[#04111c] p-5 lg:w-[310px] lg:overflow-y-auto lg:border-l lg:border-t-0 lg:[height:calc(100vh-57px)]">

            <section>
              <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-white/30">Couleur</p>
              <div className="flex flex-wrap gap-2">
                {COLORS.map(c=><button key={c.hex} title={c.name} onClick={()=>setColor(c.hex)}
                  className={`h-8 w-8 rounded-full border-2 transition ${color===c.hex?"border-[#c9a84c] scale-110":"border-white/15 hover:border-white/40"}`}
                  style={{backgroundColor:c.hex}}/>)}
              </div>
              <p className="mt-1.5 text-xs text-white/40">{colorName}</p>
            </section>

            <section className="mt-5">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-white/30">Arrière-plan</p>
              <div className="flex flex-wrap gap-2">
                {BGS.map(b=><button key={b.name} title={b.name} onClick={()=>{setBgColor(b.color);setBgName(b.name);}}
                  className={`h-8 w-8 rounded-lg border-2 transition ${bgName===b.name?"border-[#c9a84c] scale-110":"border-white/15 hover:border-white/40"}`}
                  style={{backgroundColor:b.color}}/>)}
                <input ref={bgRef} type="file" accept="image/*" onChange={()=>{}} className="hidden"/>
              </div>
              <p className="mt-1.5 text-xs text-white/40">{bgName}</p>
            </section>

            <section className="mt-5">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-white/30">Design</p>
              <input ref={logoRef} type="file" accept="image/png,image/jpeg,image/webp" onChange={uploadLogo} className="hidden"/>
              <button onClick={()=>logoRef.current?.click()}
                className="w-full rounded-xl border border-dashed border-white/15 bg-white/5 py-3 text-sm text-white/50 transition hover:border-[#c9a84c]/50 hover:text-white">
                {logoName?`✓ ${logoName}`:"Importer un logo (PNG)"}
              </button>
              {logoTex&&<div className="mt-4 space-y-3">
                <label className="block">
                  <span className="text-xs text-white/30">Taille</span>
                  <input type="range" min={0.05} max={0.35} step={0.005} value={logoScale}
                    onChange={e=>setLogoScale(parseFloat(e.target.value))} className="mt-1 w-full accent-[#c9a84c]"/>
                </label>
                <label className="block">
                  <span className="text-xs text-white/30">Position verticale</span>
                  <input type="range" min={-0.15} max={0.22} step={0.005} value={logoY}
                    onChange={e=>setLogoY(parseFloat(e.target.value))} className="mt-1 w-full accent-[#c9a84c]"/>
                </label>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-white/30">Logo dans le dos</span>
                  <button onClick={()=>setLogoBack(v=>!v)}
                    className={`relative h-6 w-11 rounded-full transition-colors ${logoBack?"bg-[#c9a84c]":"bg-white/15"}`}>
            ...
            <section className="mt-5">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-white/30">Animation</p>
              <div className="grid grid-cols-2 gap-2">
                {ANIMS.map(a=><button key={a.id} onClick={()=>setAnim(a.id)}
                  className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-xs transition ${anim===a.id?"border-[#c9a84c] bg-[#c9a84c]/15 text-[#c9a84c]":"border-white/10 bg-white/5 text-white/50 hover:border-white/30"}`}>
            ...
            <section className="mt-5">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-white/30">Exporter</p>
              <div className="space-y-2">
                <button onClick={dlPNG} className="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 text-xs font-medium text-white/60 transition hover:bg-white/10">
                  📷 Télécharger PNG
                </button>
                {!rec
                  ?<button onClick={startRec} className="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 text-xs font-medium text-white/60 transition hover:border-red-400/40 hover:text-red-400">
                    ● Enregistrer vidéo (max {MAX_REC}s)
                  </button>
                  :<button onClick={stopRec} className="w-full rounded-xl border border-red-500/40 bg-red-500/10 py-2.5 text-xs font-semibold text-red-400">
            ...
            <section className="mt-6 space-y-2">
              ...
              <p className="text-center text-[10px] text-white/20">DTF & broderie · Caractère Store · Alger</p>
            </section>
          </aside>
        </div>
      </div>
