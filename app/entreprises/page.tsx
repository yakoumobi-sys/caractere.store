  ...
  return (
    <>
      ...
      <main className="pt-14 min-h-screen">
        ...
        {/* HERO */}
        <section
          className="relative overflow-hidden px-6 pb-16 pt-16 text-center"
        ...
        >
          <div
            ...
            style={{ background: 'radial-gradient(circle, rgba(186,230,253,0.5) 0%, transparent 70%)', filter: 'blur(70px)' }}
          />
          <div className="relative z-10 mx-auto max-w-md">
            <span
              className="inline-flex items-center gap-1.5 rounded-full border px-4 py-1.5 text-xs font-medium backdrop-blur-sm mb-5"
              style={{ borderColor: 'rgba(255,255,255,0.3)', backgroundColor: 'rgba(255,255,255,0.1)', color: '#fff' }}
            >
            ...
            <h1 className="text-4xl font-extrabold leading-tight text-white mb-4">
              ...
              <br />
              <span style={{ backgroundImage: 'linear-gradient(90deg, #BAE6FD, #7DD3FC)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                Sans compromis.
              </span>
            </h1>
            ...
            <a
              ...
              target="_blank"
              className="inline-block rounded-full py-3 px-8 text-sm font-semibold no-underline"
        ...
        {/* SAVINGS BAR */}
        <div
          className="flex items-center gap-6 overflow-x-auto px-6 py-3 text-[11px] font-semibold uppercase tracking-widest"
          style={{ backgroundColor: '#0C4A6E', color: '#BAE6FD', whiteSpace: 'nowrap' }}
        >
        ...
        <section className="px-6 py-16 max-w-[1100px] mx-auto">
          <p className="text-[11px] font-bold uppercase tracking-widest mb-2 text-brand-gray">Nos offres</p>
          <h2 className="text-[28px] font-extrabold mb-10 tracking-tight text-brand-dark">Choisissez votre pack</h2>
          ...
          <div className="flex flex-col gap-4 md:grid md:grid-cols-3 md:gap-6">
            {PACKS.map((pack) => (
              <div
                ...
                style={{
                  borderColor: pack.featured ? '#0C4A6E' : 'rgba(0,0,0,0.08)',
                  background: pack.featured ? 'linear-gradient(165deg, #0C4A6E 0%, #1E6FA8 100%)' : '#fff',
                  color: pack.featured ? '#fff' : '#1d1d1f',
                  transform: pack.featured ? 'scale(1.02)' : 'none',
                  boxShadow: pack.featured ? '0 20px 50px rgba(12,74,110,0.3)' : '0 2px 12px rgba(0,0,0,0.06)',
                }}
              >
                {pack.featured && (
                  <div
                ...
                <div className="p-7">
                  <p className="text-[11px] font-bold uppercase tracking-widest mb-2"
                    style={{ color: pack.featured ? '#BAE6FD' : '#38BDF8' }}>
                    {pack.tier}
                  </p>
                  <h3 className="text-[22px] font-extrabold mb-2">{pack.name}</h3>
                  <p className="text-[13px] mb-6 leading-relaxed"
                    style={{ color: pack.featured ? 'rgba(255,255,255,0.65)' : '#6B6B6B' }}>
                    {pack.subtitle}
                  </p>

                  <div className="mb-6">
                    <p className="text-[12px] line-through mb-1"
                      style={{ color: pack.featured ? 'rgba(255,255,255,0.4)' : '#9E9E9E' }}>
                      Valeur reelle : {pack.prixReel} DA
                    </p>
                    <p className="text-[36px] font-extrabold leading-none"
                      style={{ color: pack.featured ? '#BAE6FD' : '#0C4A6E' }}>
                      {pack.prix} <span className="text-[16px] font-medium">DA</span>
                    </p>
                    <p className="text-[12px] mt-1"
                      style={{ color: pack.featured ? 'rgba(255,255,255,0.5)' : '#6B6B6B' }}>
                      {pack.articles} articles personnalises
                    </p>
                    <span
                      className="inline-block text-[11px] font-bold mt-2 px-2 py-1 rounded-full"
                      style={{ backgroundColor: 'rgba(56,189,248,0.15)', color: pack.featured ? '#BAE6FD' : '#0C4A6E' }}
                    >
                      {pack.economie}
                    </span>
                  </div>
                  ...
                  <div className="h-px my-5"
                    style={{ backgroundColor: pack.featured ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.06)' }} />

                  <p className="text-[10px] font-bold uppercase tracking-widest mb-3"
                    style={{ color: pack.featured ? 'rgba(255,255,255,0.45)' : '#9E9E9E' }}>
                    Contenu du pack
                  </p>
                  <ul className="mb-6 space-y-2">
                    {pack.items.map((item) => (
                      <li key={item.name} className="flex justify-between items-center text-[13px] py-1.5 border-b"
                        style={{ borderColor: pack.featured ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)' }}>
                  ...
                  <a
                    ...
                    target="_blank"
                    className="block text-center rounded-full py-3.5 text-[13px] font-semibold no-underline transition-all"
                    style={
                      pack.featured
                        ? { backgroundColor: '#fff', color: '#0C4A6E' }
                        : { backgroundColor: '#0C4A6E', color: '#fff' }
                    }
                  >
                    Commander ce pack →
                  </a>
        ...
        >
          <div className="max-w-[1100px] mx-auto">
            <h2 className="text-[28px] font-extrabold mb-10 text-white">
              Pourquoi choisir <span style={{ color: '#BAE6FD' }}>Caractere</span> ?
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {FEATURES.map((f) => (
                ...
                >
                  ...
                  <p className="text-[12px] leading-relaxed" style={{ color: 'rgba(255,255,255,0.65)' }}>{f.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="px-6 py-20 text-center max-w-[700px] mx-auto">
          <h2 className="text-[32px] font-extrabold leading-tight mb-4 text-brand-dark">
            Un besoin specifique ?<br />On vous fait un devis.
          </h2>
          <p className="text-[15px] leading-relaxed mb-10 text-brand-gray">
          ...
          <a
            ...
            target="_blank"
            className="inline-flex items-center gap-3 no-underline rounded-full px-8 py-4 text-[14px] font-semibold text-white transition-all"
