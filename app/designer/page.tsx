...
function Shirt({ color, logoTexture }: { color: string; logoTexture: THREE.Texture | null }) {
  ...
  return (
    <Center>
      <mesh geometry={gltf.nodes.T_Shirt_male.geometry} material={mat} dispose={null}>
        {logoTexture && (
          <Decal position={[0, 0.05, 0.15]} rotation={[0, 0, 0]} scale={0.15}>
...
function Scene({ bgColor, anim, color, logoTexture }: { bgColor: string; anim: Anim; color: string; logoTexture: THREE.Texture | null }) {
  ...
  return (
    <>
      ...
      <group ref={g}>
        <Shirt color={color} logoTexture={logoTexture} />
      </group>
...
export default function Studio3D() {
  ...
  // Charger le logo depuis localStorage si disponible
  useEffect(() => {
    try {
