...
export default function Studio3D() {
  ...
  // Charger les données du designer au montage
  useEffect(() => {
    try {
      const data = localStorage.getItem("designData");
      if (data) {
        const parsed = JSON.parse(data);
        setColor(parsed.color);
        setLogoPos(parsed.logoPos);
        setLogoName(parsed.logoName);

        if (parsed.logoTexture) {
          new THREE.TextureLoader().load(parsed.logoTexture, (tex) => {
            tex.colorSpace = THREE.SRGBColorSpace;
            tex.anisotropy = 16;
            setLogoTex(tex);
          });
        }
        if (parsed.logoTextureBack) {
          new THREE.TextureLoader().load(parsed.logoTextureBack, (tex) => {
            tex.colorSpace = THREE.SRGBColorSpace;
            tex.anisotropy = 16;
            setLogoTexBack(tex);
          });
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-white text-[#0a1f2e]">
        <header className="flex items-center justify-between border-b border-gray-100 bg-white px-5 py-3">
          ...
          <div className="flex items-center gap-3">
            <span className="rounded-full border border-[#d41717]/60 px-2 py-0.5 text-[10px] font-bold uppercase text-[#d41717]">Bêta</span>
        ...
        <div className="mx-auto flex max-w-6xl flex-col lg:flex-row">
          <div className="relative h-[56vh] min-h-[340px] flex-1 lg:h-[calc(100vh-57px)]" style={{background: bgColor.startsWith("linear") ? bgColor : undefined, backgroundColor: bgColor.startsWith("linear") ? undefined : bgColor}}>
            <Canvas camera={{ position: [0, 0, 2.2], fov: 25 }} gl={{ preserveDrawingBuffer: true }}
          ...
          <aside className="w-full border-t border-gray-100 bg-[#f8fafc] p-5 lg:w-[310px] lg:overflow-y-auto lg:border-l lg:border-t-0 lg:[height:calc(100vh-57px)]">
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {COLORS.map(c=><button key={c.hex} title={c.name} onClick={()=>setColor(c.hex)}
                  className={`h-8 w-8 rounded-full border-2 transition ${color===c.hex?"border-[#d41717] scale-110":"border-gray-300"}`}
            ...
            <section className="mt-5">
              ...
              <div className="space-y-2">
                ...
                {!rec
                  ?<button onClick={startRec} className="w-full rounded-xl border border-gray-200 bg-white py-2.5 text-xs font-medium text-[#0a1f2e]/60">
                    ● Vidéo
                  </button>
